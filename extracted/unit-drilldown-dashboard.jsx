...
export default function UnitDrilldownDashboard() {
  const units = useMemo(() => {
    const m = {};
    RECORDS.forEach(r => {
  ...
  const selectedByCategory = useMemo(() => {
    const m = {};
    selectedRecords.forEach(r => { m[r.category] = (m[r.category] || 0) + r.cost; });
  ...
  return (
    <div className="app">
      <style>{`
        ...
        .head-total { font-family: 'IBM Plex Mono', monospace; font-size: 13px; color: var(--ink-soft); }
        .cards { display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 10px; margin-bottom: 20px; }
        .card { background: var(--surface); border: 1.5px solid var(--line); border-radius: 8px; padding: 12px; cursor: pointer; text-align: left; transition: border-color .1s; }
        ...
        .chart-title { font-size: 13px; font-weight: 600; margin: 4px 0 8px; }
      ...
      <div className="head">
        <h2>Fleet spend by unit</h2>
        <span className="head-total">${grandTotal.toLocaleString(undefined, { minimumFractionDigits: 2 })} total across {units.length} units</span>
      </div>
