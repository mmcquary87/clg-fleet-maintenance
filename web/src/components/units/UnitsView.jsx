...
import { Button, Card, Badge, Eyebrow, Alert } from "../../ds";
...
export default function UnitsView() {
  ...
  const visible = units.filter((u) => filter === "all" || (filter === "active" ? u.is_active : !u.is_active));
      ...
      <Card padding={0}>
        {loading ? (
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "var(--clg-size-small)" }}>
            <thead>
              <tr>
                {["Unit", "Type", "VIN", "Location", "Status", ""].map((h) => (
                  <th key={h} style={{
